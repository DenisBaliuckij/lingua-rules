# lingua-rules — usage guide

This guide shows how to run lingua-rules with plain `python` (no shell aliases,
...
In the rest of this guide, `python` means that interpreter:
...
![Languages page](images/01-index.png)
...
![Category page before the regular rule](images/02-category-before.png)
...
![Rejected feature value](images/03-new-rule-error.png)

With a valid key (`number=plural`, suffix `en`):

![Add a rule form](images/04-new-rule-form.png)
...
![Category page after adding the rule](images/05-category-after.png)

**Step 5 — try it.** A regular noun uses the new rule; an exception uses its
override:

![Try it: boek](images/06-try-it-regular.png)

![Try it: kind](images/07-try-it-exception.png)
...
![Test results](images/08-tests.png)
